...
export const enroll = async ( userId : string, courseId: any) => {
    const { data } = await axiosWithCredentials.put(`${ENROLL_API}/toggleEnroll/${userId}/${courseId}`);
}

export const unenroll = async ( userId : string, courseId: any) => {
    const { data } = await axiosWithCredentials.put(`${ENROLL_API}/toggleUnnroll/${userId}/${courseId}`);
}
